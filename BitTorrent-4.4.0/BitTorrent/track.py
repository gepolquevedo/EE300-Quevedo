...
defaults = [
    ...
    ('pid', '/var/run/bittorrent-tracker.pid',
     "Path to PID file"),
    ('strict_locality',0,_('determines if tracker will employ localization (0==none, 1=standard, 2==strict)'))	
    ] 
...
class Tracker(object):
        ...
        if event == 'stopped':
            ...
            if not left and peer['left']:
                ...
                if not peer.get('nat', -1):
                    for n in range(256):
			try:
                        	for bc in self.becache[n][infohash]:
                        	    bc[1][myid] = bc[0][myid]
                        	    del bc[0][myid]
			except KeyError:
				continue
    ...
    def peerlist(self, infohash, stopped, is_seed, return_type, rsize, ip):
        data = {}    # return data
	...
	len_l = len(bc[0][0])
        ...
        if not (len_l+len_s):   # caches are empty!
            data['peers'] = []
            return data
        l_get_size = int(float(rsize)*(len_l)/(len_l+len_s))  #what is this formula?
...
#        self.table format : [group1, group2,...]
    ...
    def _add_seed(self, group):
        
        #bc = self.becache[group].setdefault(infohash,[[{}, {}], [{}, {}], [{}, {}]])
        for item in self.onlinesources.iteritems(): 
            infohash = item[0]
            ip = item[1]['ip']
            port = item[1]['port']
	    peerid = item[1]['peer id']
            not_seed = item[1]['not seed']		
