...
class PiecePicker(object):
    ...
    def _next_mod(self, havefunc, seed):
	bests = []
	if seed:
            s = self.seedstarted
        else:
	...
	print 'seed',self.seedstarted
	print 'noneseed',self.started	
	mod_i = self._next_mod(havefunc, seed)
	return mod_i        
	bests = None
        bestnum = 2 ** 30
	if seed:
